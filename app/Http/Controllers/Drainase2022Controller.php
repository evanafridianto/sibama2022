...
class Drainase2022Controller extends Controller
{
    ...
    {
        $drainase2022 = Drainase2022::find($id);
        $title = 'Edit Data';
        $kecamatan = Kecamatan::all();
        $kelurahan = Kelurahan::all();
        $jalan = Jalan::all();
        $fisik = Kategori::where('induk', 'Kondisi Fisik')->get();
        return view('pages.drainase2022.form', compact(['title', 'kecamatan', 'kelurahan', 'jalan', 'fisik', 'drainase2022']));
    }
        ...
        if ($validator->fails()) {
            ...
            if (empty($request->id)) { //CREATE
                ...
                try {
                    ...
                    $drainase2022->save();
                    DB::commit();
                    return response()->json(['status' => true, 'redirect' =>  route("drainase2022.edit", $request->id)]);
                } catch (\Throwable $th) {
                    DB::rollBack();
                }
            }
